...
metadata_args_strategy = st.fixed_dictionaries({
    ...
    "database_alias_key": st.one_of(
        st.just("TABLE_CATALOG"),
        st.just("DATABASE_NAME"),
        st.just("CATALOG_NAME")
    ),
    "schema_alias_key": st.one_of(
        st.just("TABLE_SCHEMA"),
        st.just("SCHEMA_NAME")
    ),
...
sql_column_strategy = st.fixed_dictionaries({
    ...
    "type": st.sampled_from(["VARCHAR", "INTEGER", "FLOAT", "TIMESTAMP", "BOOLEAN"]),
...
sql_query_strategy = st.one_of(
    ...
    st.builds(
        lambda table: f"SELECT * FROM {table}",
