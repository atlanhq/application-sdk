...
schema_selection_strategy = st.one_of(
    schema_list_strategy,
    wildcard_schema_strategy
)

# Strategy for generating metadata entries as tuples (hashable)
metadata_entry_tuple_strategy = st.tuples(
...
metadata_list_strategy = st.lists(
    metadata_entry_tuple_strategy,
    min_size=1,
    max_size=10,
    unique=True
).map(lambda entries: [{"TABLE_CATALOG": db, "TABLE_SCHEMA": schema} for db, schema in entries])
...
mixed_mapping_strategy = st.one_of(
    db_schema_mapping_strategy,
    regex_db_schema_mapping_strategy
)
