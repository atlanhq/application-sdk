...
temporal_config_strategy = st.fixed_dictionaries({
    ...
    "application_name": st.text(min_size=3, max_size=50, alphabet=st.characters(whitelist_categories=('Ll', 'Lu', 'Nd', '_', '-'))),
    "namespace": st.one_of(
        st.just("default"),
        st.text(min_size=3, max_size=20, alphabet=st.characters(whitelist_categories=('Ll', 'Lu', 'Nd', '_', '-')))
    )
})
...
worker_config_strategy = st.fixed_dictionaries({
    ...
    "sticky_queue_schedule_to_start_timeout": st.integers(min_value=1, max_value=60),
    ...
    "max_task_queue_activities_per_second": st.integers(min_value=1, max_value=1000),
