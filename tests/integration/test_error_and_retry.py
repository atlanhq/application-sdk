...
class RetryApp(App):
    ...
    async def do_work(self, input: RetryInput) -> RetryOutput:
        _attempt_counters[input.test_id] = _attempt_counters.get(input.test_id, 0) + 1
...
class CustomRetryApp(App):
    @task(retry_policy=_custom_policy)
...
async def test_non_retryable_error_stops_immediately(
    ...
    reregister_app(NonRetryableApp)
    ...
    # "Deterministic failure" is in Temporal's cause chain, not the top-level
    # WorkflowFailureError message — walk the chain to verify.
    cause: BaseException | None = exc_info.value
    while cause is not None:
        if "Deterministic failure" in str(cause):
            break
        cause = getattr(cause, "cause", None) or cause.__cause__
    assert cause is not None, "Expected 'Deterministic failure' in exception chain"
...
@pytest.mark.integration
async def test_custom_retry_policy_propagation(run_worker, executor, reregister_app):
    """P2.3: Custom RetryPolicy with max_attempts=2 retries once and succeeds."""
    test_id = uuid4().hex
    _attempt_counters[test_id] = 0

    reregister_app(CustomRetryApp)
    async with run_worker():
