...
async def test_on_complete_called_after_success(run_worker, executor, reregister_app):
    """P4.1: on_complete() is invoked after a successful run()."""
    _on_complete_log.clear()
    reregister_app(SuccessLifecycleApp)
...
@pytest.mark.integration
async def test_on_complete_called_after_failure(run_worker, executor, reregister_app):
    """P4.2: on_complete() is invoked even when run() raises an exception."""
    _on_complete_log.clear()
    reregister_app(FailLifecycleApp)
    async with run_worker():
...
@pytest.mark.integration
async def test_context_properties_populated(run_worker, executor, reregister_app):
    """P4.3: self.context exposes app_name, run_id, correlation_id in a @task."""
    reregister_app(ContextApp)
    async with run_worker():
