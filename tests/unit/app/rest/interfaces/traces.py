...
#     # Create a mock TracesData object (OpenTelemetry traces)
#     mock_traces_data = mock.Mock()
    
#     mock_span = mock.Mock(
#         name="trace_3",
#         start_time_unix_nano=int(datetime(2023, 1, 3, 12, 0, 0, tzinfo=UTC).timestamp() * 1e9),
#         end_time_unix_nano=int(datetime(2023, 1, 3, 12, 5, 0, tzinfo=UTC).timestamp() * 1e9),
#         trace_id=b'1234567890123456',
#         span_id=b'6543210987654321',
#         parent_span_id=b'0000000000000000',
#         kind=1,
#         attributes=[mock.Mock(key="attr3", value=mock.Mock(string_value="value3"))],
#         events=[
#             mock.Mock(
#                 name="event_1",
#                 time_unix_nano=int(datetime(2023, 1, 3, 12, 1, 0, tzinfo=UTC).timestamp() * 1e9),
#                 attributes=[mock.Mock(key="event_attr1", value=mock.Mock(string_value="event_value1"))],
#             )
#         ]
#     )
    
#     mock_scope_span = mock.Mock(spans=[mock_span])
#     mock_resource_span = mock.Mock(resource=mock.Mock(attributes=[]), scope_spans=[mock_scope_span])
#     mock_traces_data.resource_spans = [mock_resource_span]

#     # Call the method
#     created_traces = Traces.create_traces(session, mock_traces_data)

#     # Assert the correct trace was created
#     assert len(created_traces) == 1
#     assert str(created_traces[0].name) == "trace_3"
#     assert str(created_traces[0].trace_id) == "1234567890123456"
#     assert str(created_traces[0].events[0]["name"]) == "event_1"
