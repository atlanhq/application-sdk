...
class TestObjectStoreOutput:
    ...
    async def test_push_file_to_object_store_success(self, mock_dapr_client: MagicMock) -> None:
        # Setup
        ...
        with patch("builtins.open", mock_file):
            await ObjectStoreOutput.push_file_to_object_store(
                ...
                file_path="/test/prefix/test.txt"
            )
    ...
    @patch("application_sdk.outputs.objectstore.DaprClient")
    async def test_push_file_to_object_store_file_error(self, mock_dapr_client: MagicMock) -> None:
        # Setup
        mock_client = MagicMock()
        mock_dapr_client.return_value.__enter__.return_value = mock_client
    ...
    @patch("application_sdk.outputs.objectstore.DaprClient")
    async def test_push_files_to_object_store_success(self, mock_dapr_client: MagicMock) -> None:
        # Setup
        mock_client = MagicMock()
        mock_dapr_client.return_value.__enter__.return_value = mock_client
        ...
        with patch("os.walk") as mock_walk, \
            ...
            mock_walk.return_value = [
                ("/test/input", [], list(test_files.keys()))
            ]
            
            await ObjectStoreOutput.push_files_to_object_store(
                output_prefix="/test/prefix",
                input_files_path="/test/input"
            )
    ...
    @patch("application_sdk.outputs.objectstore.DaprClient")
    async def test_push_files_to_object_store_invalid_directory(self, mock_dapr_client: MagicMock) -> None:
        with patch("os.path.isdir") as mock_isdir:
            ...
            with pytest.raises(ValueError, match="not a valid directory"):
                await ObjectStoreOutput.push_files_to_object_store(
                    output_prefix="/test/prefix",
                    input_files_path="/invalid/path"
                )


class TestObjectStoreInput:
    @patch("application_sdk.inputs.objectstore.DaprClient")
    def test_download_file_from_object_store_success(self, mock_dapr_client: MagicMock) -> None:
        # Setup
        mock_client = MagicMock()
        mock_dapr_client.return_value.__enter__.return_value = mock_client
        ...
        with patch("builtins.open", mock_file):
            ObjectStoreInput.download_file_from_object_store(
                download_file_prefix="/test/prefix",
                file_path="/test/prefix/test.txt"
            )
    ...
    @patch("application_sdk.inputs.objectstore.DaprClient")
    def test_download_file_from_object_store_error(self, mock_dapr_client: MagicMock) -> None:
        # Setup
        mock_client = MagicMock()
        mock_dapr_client.return_value.__enter__.return_value = mock_client
