...
class TestPreflightCheckStatus:
    """Per-check verdict (HYP soft-fail): status records blocking intent."""

    def test_defaults_to_none_for_failed_check(self):
        # Back-compat: un-migrated handlers produce status=None on failures;
        # the gate warns and emits "unset" instead of guessing intent.
        check = PreflightCheck(name="auth", passed=False)
        assert check.status is None

    def test_derived_ready_for_passed_check(self):
        # A passed check needs no stamp — READY is the only sensible verdict.
        check = PreflightCheck(name="auth", passed=True)
        assert check.status is PreflightStatus.READY

    def test_explicit_blocking_failure(self):
        check = PreflightCheck(
            name="auth", passed=False, status=PreflightStatus.NOT_READY
        )
        assert check.status is PreflightStatus.NOT_READY

    def test_explicit_advisory_failure(self):
        check = PreflightCheck(
            name="tablesCheck", passed=False, status=PreflightStatus.PARTIAL
        )
        assert check.status is PreflightStatus.PARTIAL

    def test_explicit_ready_on_passed_check(self):
        check = PreflightCheck(name="auth", passed=True, status=PreflightStatus.READY)
        assert check.status is PreflightStatus.READY

    def test_ready_on_failed_check_rejected(self):
        with pytest.raises(ValidationError):
            PreflightCheck(name="auth", passed=False, status=PreflightStatus.READY)

    def test_blocking_status_on_passed_check_rejected(self):
        with pytest.raises(ValidationError):
            PreflightCheck(name="auth", passed=True, status=PreflightStatus.NOT_READY)

    def test_advisory_status_on_passed_check_rejected(self):
        with pytest.raises(ValidationError):
            PreflightCheck(name="auth", passed=True, status=PreflightStatus.PARTIAL)

    def test_serializes_as_string_value(self):
        check = PreflightCheck(
            name="auth", passed=False, status=PreflightStatus.NOT_READY
        )
        assert check.model_dump(mode="json")["status"] == "not_ready"

    def test_none_status_excluded_from_exclude_none_dump(self):
        # The HTTP summary uses model_dump(exclude_none=True); an unset status
        # must disappear rather than serialize as null.
        check = PreflightCheck(name="auth", passed=False)
        assert "status" not in check.model_dump(mode="json", exclude_none=True)
    ...
    def test_accepts_wire_string(self):
        check = PreflightCheck(name="auth", passed=False, status="partial")
        assert check.status is PreflightStatus.PARTIAL
