...
def test_process_column_name(sql_transformer):
    ...
    assert sql_transformer._process_column_name("column.with.dots") == '"column.with.dots"'
...
def test_get_columns(sql_transformer, sample_dataframe, sample_yaml_template):
    ...
    assert 'concat(connection_qualified_name, \'/\', table_catalog, \'/\', table_schema, \'/\', table_name) AS "attributes.qualifiedName"' in columns
    assert 'case when table_type = \'TABLE\' then \'table\' when table_type = \'VIEW\' then \'view\' else table_type end AS "attributes.type"' in columns
    assert '"attributes.literal" AS "attributes.literal"' in columns
    assert {"name": '"attributes.literal"', "source_query": "'Database'"} == literal_columns[0]


@patch("builtins.open", new_callable=mock_open)
@patch("yaml.safe_load")
def test_generate_sql_query(mock_yaml_load, mock_file, sql_transformer, sample_dataframe, sample_yaml_template):
    """Test the generate_sql_query method"""
    mock_yaml_load.return_value = sample_yaml_template
    default_attributes = {}
    result, literal_columns = sql_transformer.generate_sql_query("dummy_path", sample_dataframe, default_attributes)
...
def test_build_struct(sql_transformer):
    ...
    level = {
        "columns": [("attributes.name", "name"), ("attributes.qualifiedName", "qualifiedName")],
        "nested": {
            "columns": [("nested.column", "column")]
        }
    }
...
def test_get_grouped_dataframe_by_prefix(sql_transformer):
    ...
    df = daft.from_pydict({
        ...
        "attributes.qualifiedName": ["conn1/db1/schema1/table1", "conn1/db1/schema2/table2", "conn1/db1/schema3/table3"],
        "attributes.database.typeName": ["Database", "Database", "Database"],
        "attributes.database.uniqueAttributes.qualifiedName": ["conn1/db1", "conn1/db1", "conn1/db1"],
    ...
    expected_schema = [
        ...
        Field.create(name="attributes", dtype=daft.DataType.struct({"name": daft.DataType.string(), "qualifiedName": daft.DataType.string(), "type": daft.DataType.string(), "kind": daft.DataType.string(), "isPartition": daft.DataType.bool(), "partitionStrategy": daft.DataType.string(), "viewDefinition": daft.DataType.string(), "database": daft.DataType.struct({"typeName": daft.DataType.string(), "uniqueAttributes": daft.DataType.struct({"qualifiedName": daft.DataType.string()})})})),
        Field.create(name="customAttributes", dtype=daft.DataType.struct({"parent_name": daft.DataType.string()}))
    ]
...
def test_prepare_template_and_attributes(mock_generate, sql_transformer, sample_dataframe):
...
def test_transform_metadata_empty_dataframe(sql_transformer):
    ...
    empty_df = daft.from_pydict({"dummy": []})  # Add a dummy column to avoid empty schema
...
@patch("application_sdk.transformers.sql.SQLTransformer._prepare_template_and_attributes")
@patch("application_sdk.transformers.sql.SQLTransformer._get_grouped_dataframe_by_prefix")
def test_transform_metadata(mock_group, mock_prepare, sql_transformer, sample_dataframe):
