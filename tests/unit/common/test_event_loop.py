...
class TestEventLoop(unittest.TestCase):
    ...
    def test_event_loop_performance(self):
        ...
        loop.run_until_complete(asyncio.gather(*[self.async_operation() for _ in range(1000)]))
        ...
        print(f"Speed improvement: {((default_time - uvloop_time) / default_time) * 100:.2f}%")

    @patch('application_sdk.application.fastapi.Server')
    def test_fastapi_event_loop(self, mock_server):
        """Test the event loop used by FastAPI application"""
        ...
        app = FastAPIApplication(handler=MockHandler())
        
        # Get the current event loop
        loop = asyncio.get_event_loop()
        loop_class = loop.__class__.__name__
        
        print(f"\nFastAPI application event loop: {loop_class}")
        print(f"Event loop module: {loop.__class__.__module__}")

        # Test with uvloop
        uvloop.install()
        loop = asyncio.get_event_loop()
        new_loop_class = loop.__class__.__name__
        
        print(f"FastAPI application event loop after uvloop install: {new_loop_class}")
        print(f"Event loop module after uvloop install: {loop.__class__.__module__}")
