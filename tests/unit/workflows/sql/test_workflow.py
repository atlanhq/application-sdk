...
async def test_fetch_and_transform():
    ...
    with patch('temporalio.workflow.execute_activity_method', side_effect=[mock_fetch.return_value] + [mock_transform.return_value] * 2):
    ...
    assert mock_fetch.call_count == 0  # Not called directly due to mocking execute_activity_method
    
    # Verify transform was called for each batch
    assert mock_transform.call_count == 0  # Not called directly due to mocking execute_activity_method
...
async def test_fetch_and_transform_error_handling():
    ...
    mock_fetch_invalid = AsyncMock(return_value=ActivityStatistics(
        total_record_count=10,
        chunk_count=2,
        typename=None
    ).model_dump())
...
async def test_run():
    ...
    with patch('temporalio.workflow.info', return_value=mock_info), \
         patch.object(workflow, 'fetch_and_transform') as mock_fetch_transform:
        ...
        assert mock_fetch_transform.call_count == 4  # databases, schemas, tables, columns
