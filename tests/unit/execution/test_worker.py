...
class _FakeInnerWorker:
    """Mimics the temporalio Worker lifecycle used by run_until_shutdown.

    ``run()`` blocks until ``shutdown()`` is called (like the real worker),
    unless ``return_immediately`` is set — which reproduces a run loop that
    exits on its own (the zombie failure mode).
    """
    ...
    async def run(self) -> None:
        ...
        if not self._return_immediately:
...
class TestRunUntilShutdown:
    """AppWorker.run_until_shutdown: race the poll loop against the signal."""

    @staticmethod
    def _app_worker(inner: _FakeInnerWorker) -> AppWorker:
        return AppWorker(inner, start_event_params=_MINIMAL_START_PARAMS)  # type: ignore[arg-type]

    def test_is_run_loop_alive_before_start(self) -> None:
        app_worker = self._app_worker(_FakeInnerWorker())
        # No run task yet — treated as alive/starting so the probe doesn't
        # false-fail before the worker has been supervised.
        assert app_worker.is_run_loop_alive() is True

    @pytest.mark.asyncio
    async def test_returns_on_shutdown_signal(self) -> None:
        inner = _FakeInnerWorker()
        app_worker = self._app_worker(inner)
        ...
        with (
            ...
            mock.patch(DRAIN_DELAY_PATCH, 0),
        ):
            asyncio.create_task(_signal_later())
            await app_worker.run_until_shutdown(shutdown_event)

        assert inner.shutdown_called is True
        # Run loop has finished draining — probe now reports not alive.
        assert app_worker.is_run_loop_alive() is False

    @pytest.mark.asyncio
    async def test_raises_when_run_loop_exits_without_signal(self) -> None:
        from application_sdk.execution._temporal._activity_errors import (
            WorkerRunLoopExited,
        )

        inner = _FakeInnerWorker(return_immediately=True)
        app_worker = self._app_worker(inner)
        shutdown_event = asyncio.Event()
        ...
        with mock.patch(
            ...
            with pytest.raises(WorkerRunLoopExited):
                await app_worker.run_until_shutdown(shutdown_event)
    ...
    @pytest.mark.asyncio
    async def test_reraises_run_loop_exception_without_signal(self) -> None:
        boom = RuntimeError("poll stream broke")
        inner = _FakeInnerWorker(run_exc=boom, return_immediately=True)
        app_worker = self._app_worker(inner)
        shutdown_event = asyncio.Event()
        ...
        with mock.patch(
            ...
            with pytest.raises(RuntimeError, match="poll stream broke"):
                await app_worker.run_until_shutdown(shutdown_event)
