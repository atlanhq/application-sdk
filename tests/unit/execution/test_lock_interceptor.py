...
class TestStartActivityWithLock:
    ...
    async def test_missing_schedule_to_close_raises(self):
        ...
        from application_sdk.execution._temporal._lock_errors import (
            MissingScheduleToCloseTimeoutError,
        )
        ...
        with (
            ...
            pytest.raises(MissingScheduleToCloseTimeoutError) as exc_info,
        ):
            await interceptor.start_activity(inp)
        assert exc_info.value.code == "INVALID_INPUT_LOCK_TIMEOUT_REQUIRED"
