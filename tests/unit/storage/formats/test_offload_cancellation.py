...
class TestReaderCancellation:
    ...
    async def test_cancelling_mid_decode_does_not_close_under_the_worker(
        ...
        Before FND-315 the generator's ``finally`` called ``pf.close()``
        inline, so a cancellation landing at the offload ``await`` closed the
        handle while the orphaned worker was still inside ``next(batches)``,
        decoding from it.
        """
    ...
    async def test_the_orphaned_worker_closes_the_handle_it_owns(
        self, tmp_path: Path
    ) -> None:
        """Not closing during the race must not mean never closing.

        The orphan is the last holder of the handle, so the orphan performs the
        close — off the event loop, exactly once, and with no decode in flight.
        """
        gated = gated_reader(tmp_path)
        ...
        ), "the orphaned worker never closed the handle it was left holding"
        ...
        ), "the orphan's own thread must issue the close, not the event loop"

    async def test_draining_a_file_closes_it_on_the_worker(
        self, tmp_path: Path
    ) -> None:
    ...
    async def test_closing_the_generator_early_closes_on_the_loop(
        self, tmp_path: Path
    ) -> None:
        """A consumer that stops early still gets a deterministic close.

        With no worker in flight there is nothing to race, so the close runs
        inline on the loop — one cheap syscall, and never an ``await`` in a
        ``finally`` that cancellation could skip and leak the handle.
        """
        gated = gated_reader(tmp_path, rows=12, chunk_size=4)
        gated.may_finish.set()
        ...
        assert len(gated.closes) == 1, f"expected exactly one close: {gated.closes}"
        assert gated.closes[0].decoding_in_flight is False
        assert gated.closes[0].thread_name == threading.current_thread().name
...
def make_writer(path: Path) -> ParquetFileWriter:
    ...
    attempts land on the same output directory: the anonymous
    ``defer_uploads`` path would give each writer its own ``_parquet_*``
    subdirectory and hide the race this module is about.
    """
...
class TestWriterAttemptIsolation:
    ...
    async def test_cleanup_cannot_delete_another_attempts_live_directory(
        ...
        The old cleanup did ``rmtree(..., ignore_errors=True)`` on a shared
        path, so a retry could delete the directory an orphaned writer was
        mid-write into and report nothing.
        """
    ...
    async def test_an_orphaned_writer_cannot_corrupt_the_retrys_consolidation(
        ...
        either writes, so on a shared directory both name it ``chunk-0`` and
        the orphan's late write silently replaces the retry's — wrong data, no
        error. Here each writes into its own tree, so the retry consolidates
        ...
        def gated_write_table(table, where, **kwargs):
            # Gate only the orphan's own writes; the retry must run at full
            # speed so it provably reaches consolidation first.
            if orphan._temp_run_id in str(where):
                write_started.set()
        ...
        with patch.object(pq, "write_table", side_effect=gated_write_table):
            ...
            may_write.set()
            orphan_chunk = os.path.join(
                orphan._get_temp_folder_path(0), "chunk-0.parquet"
            )
            assert await wait_until(
                lambda: os.path.exists(orphan_chunk)
