...
# ``_sha256_hex_file`` / ``_sha256_hex_file_async`` moved to
# ``storage.integrity.sha256_file`` — the one implementation every transfer
# path shares. Its tests (including the event-loop-blocking regression from
# the dbt:process heartbeat-timeout RCA) live in ``test_integrity.py``.
...
class TestMaterializeFileReference:
    ...
    async def test_directory_materialize(self, store, tmp_path) -> None:
        ...
        # Sidecars under the prefix are excluded from the listing (they are
        # not data objects) but are consumed as the integrity expectation for
        # the object they sit next to, so this one must carry a real digest.
        await _put(
            "dirkey/a.txt.sha256",
            _hash_bytes(b"alpha").encode(),
            store,
            normalize=False,
        )
