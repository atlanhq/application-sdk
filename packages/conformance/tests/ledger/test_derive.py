...
@pytest.mark.parametrize(
    ...
    [
        ("atlan-bigquery-app", "bigquery:crawler", "crawler"),
        ("atlan-tableau-app", "tableau:process-metadata", "process_metadata"),
        # bare connector name = a single-workflow app, whose workflow is run()
        ("atlan-mssql-app", "mssql", "run"),
        ("atlan-bigquery-app", "PublishWorkflow", None),
        ("atlan-bigquery-app", "QueryIntelligenceWorkflow", None),
        ("atlan-bigquery-app", "LineageWorkflow", None),
        ("atlan-bigquery-app", "", None),
        # a prefix belonging to a different connector is not ours
        ("atlan-bigquery-app", "tableau:extract-metadata", None),
    ],
)
...
def test_app_name_is_not_used_as_the_discriminator(tmp_path):
    """tableau routes its own nodes through automation-engine; salesforce ships
    ...
    _manifest(
        ...
        "app/generated/manifest.json",
        {
            "process": {
                ...
                "inputs": {"workflow_type": "tableau:process-metadata"},
            },
            "publish": {
                "app_name": "publish",
    ...
    boundary = discover_boundary(tmp_path, "atlan-tableau-app")
...
def test_per_manifest_nodes_do_not_collide(tmp_path):
    ...
    _manifest(
        ...
        {"extract": {"inputs": {"workflow_type": "bigquery:crawler"}}},
    )
    _manifest(
        tmp_path,
        "app/generated/miner/manifest.json",
        {"extract": {"inputs": {"workflow_type": "bigquery:miner"}}},
    )
    mapping = _dag_nodes_to_workflow(tmp_path, "atlan-bigquery-app")
...
def test_connector_stem_variants_are_accepted(tmp_path):
    """Manifests name the app variously: salesforce ships `salesforce-app:`."""
    _manifest(
        tmp_path,
        "app/generated/manifest.json",
        {"extract": {"inputs": {"workflow_type": "salesforce-app:extract-metadata"}}},
    )
    mapping = _dag_nodes_to_workflow(tmp_path, "atlan-salesforce-app")
