"""Meta-tests for the preflight-gate checks (P032–P035, P046, BLDX-1545, FND-901).
...
def test_rule_metadata() -> None:
    for rid in ("P033", "P034", "P035", "P046"):
        rule = get_rule(rid)
...
# ── P046 PreflightFailureLoggedAsWarning ───────────────────────────────────────


def test_p046_fires_on_logger_warning(tmp_path: Path) -> None:
    src = _handler_with_preflight(
        '        logger.warning("auth check failed: %s", "boom")\n'
        "        return PreflightOutput(checks=[])\n"
    )
    assert [f.rule_id for f in _scan(tmp_path, {"h.py": src})] == ["P046"]


def test_p046_fires_on_self_logger_and_stdlib_logging(tmp_path: Path) -> None:
    src = _handler_with_preflight(
        '        self.logger.warning("degraded")\n'
        '        logging.warning("degraded")\n'
        "        return PreflightOutput(checks=[])\n"
    )
    assert [f.rule_id for f in _scan(tmp_path, {"h.py": src})] == ["P046", "P046"]


def test_p046_silent_on_other_levels(tmp_path: Path) -> None:
    src = _handler_with_preflight(
        '        logger.info("probing")\n'
...
def test_p046_silent_outside_preflight_check(tmp_path: Path) -> None:
    src = (
        _HANDLER_IMPORTS
        + "class H(Handler):\n"
        + "    async def preflight_check(self, input: PreflightInput) -> PreflightOutput:\n"
...
def test_p046_silent_on_non_logger_receiver(tmp_path: Path) -> None:
    src = _handler_with_preflight(
...
def test_p046_suppressed(tmp_path: Path) -> None:
    src = _handler_with_preflight(
        "        # conformance: ignore[P046] advisory-only probe, see FND-901\n"
        '        logger.warning("advisory")\n'
        "        return PreflightOutput(checks=[])\n"
    )
    findings = _scan(tmp_path, {"h.py": src})
    assert [f.rule_id for f in findings] == ["P046"]
    assert findings[0].suppressed is True
