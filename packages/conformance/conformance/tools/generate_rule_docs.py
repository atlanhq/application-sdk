...
_SERIES_META: list[SeriesMeta] = [
    ...
    SeriesMeta(
        ...
        checker=(
            ...
            "`suite.checks.e2e_workflow_shape` (T020-T022), "
            "`suite.checks.e2e_generated_harness` (T023-T024), and "
            "`suite.checks.entrypoint_e2e_coverage` (T025) (AST/TOML/YAML-based)"
        ),
        suppression_example=(
    ...
    SeriesMeta(
        ...
        checker=(
            ...
            "`suite.checks.manifest_contract` (K006/K015, cross-references "
            "``app/generated/**/manifest.json`` against Python ``Output`` contracts "
            "and the SDK ``App``'s ``legacy_workflow_types`` declaration)"
        ),
        suppression_example=(
...
def _render_rule_block(rule: RuleDefinition) -> list[str]:
    ...
    # Metadata row.  ``fix_locus`` is exception-only — unset means "the source
    # of the repo under scan", which ``Scope`` already said — so the segment is
    # omitted rather than printed as a default.  Printing it every time is what
    # made it stop being read.
    metadata = [
        f"**Tier:** {_tier_badge(rule.tier)}",
        f"**Scope:** `{rule.scope.value}`",
    ]
    if rule.fix_locus is not None:
        metadata.append(f"**Fix belongs in:** `{rule.fix_locus.value}`")
    metadata.extend(
        [
