...
keep qualifying lanes under `tests/e2e/`. What matters is what the lane
exercises, not which directory it sits in.
...
draft of this ledger, where one connector's row cited the whole `tests/e2e/`
directory and so swept in `sdr/test_connector_full_dag.py`, a post-publish lane.
...
`app_name` is deliberately **not** used to find the boundary: some apps route
their own process/lineage/post-publish nodes through `automation-engine`, and
some ship an unresolved `{app_name}` placeholder. `workflow_type` is the
...
- A Temporal interceptor sees only a subset of connectors that route through
  the Temporal client. Several others test in-process, below the App layer.
- A `@task`/`@entrypoint` decorator wrapper fixes none of those: one
  connector's suite makes zero App-layer calls, another's rigorous lanes are
  helper-level, and a third's replay harness monkeypatches `upload_to_atlan`
...
- Reading `expected_data` / `schema_base_path` out of SDK `Scenario` artifacts
  ...
  harnesses, and its denominator is gameable — one connector scores 18% on it
  while having the second-best posture, and could nearly double that by
  *deleting* its shallow auth scenarios.
...
connector-alpha 4 workflows, connector-golf 4, connector-charlie 2,
connector-echo 5, connector-juliet 1, connector-delta 1, connector-bravo 1.

`tests/fixtures/sample-lanes.toml` holds a hand-classification of the
...
declared: connector-alpha 3/3 and connector-bravo 1/1, everything else 0. The
finding that matters more than the number: **the fleet's best validation
machinery is built and switched off.** connector-charlie's schema suites,
connector-delta's Pandera lanes, connector-echo's Pandera suites and
connector-golf's 14 live suites all exist and all sit behind labels, env
flags, or no CI job at all. The cheapest points on the board are un-gating
actions, not writing tests:

1. Nightly cron for connector-charlie's full mode (`mode: full` /
   `E2E_RUN_WORKFLOW=1`)
2. Nightly cron for connector-delta with `E2E_RUN_WORKFLOW=1`
3. Wire connector-golf's t-suites into a scheduled job, and populate
   `tests/replay/registry.yaml` — the nightly replay currently runs a synthetic
   micro smoke against an empty registry
4. Un-skip connector-india's baseline scenario (capture the baseline)
5. Add one connector-juliet workflow scenario with a schema directory
...
- **Config / filter / auth-axis coverage.** Unmeasurable fairly across contracts
  ...
  per-class normalization pays a premium for contract poverty (a sparse-contract
  connector would bank full marks for one hidden single-value enum while a
  rich-contract connector needs five live IdP tenants). Auth appears as an informational column only, never in the
  number.
- **Data-shape corpus adequacy** — e.g. a case where Jinja2 `striptags`
