...
| `atlan/canonicalReference` | `string` | no | The reference-app file that already has the compliant shape (`RuleDefinition.canonical_reference`); present on every app-facing rule so the remediation model knows which of `atlan-mysql-app` / `atlan-metabase-app` / `atlan-openapi-app` / `atlan-hello-world-app` to open before fixing |
