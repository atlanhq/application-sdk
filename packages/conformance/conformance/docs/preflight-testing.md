...
F016 applies to an app that defines its own `preflight_check`. It reads the tests statically. A scenario is defined when a pytest-collected test — a `test*` function, or a `test*` method of a `Test*` class, in a `test_*.py` or `*_test.py` module under `tests/` — carries `pytest.mark.preflight_conformance(rule="F016", scenario="healthy", entrypoint="crawler")` and calls the contract assertion the scenario needs. `entrypoint` defaults to `"default"`. The authoritative scenario names are in `conformance.preflight_testing.SCENARIOS`. Each `@entrypoint` the app declares needs the complete matrix; an app with none owes it once, for `default`.

The marker can decorate the test directly, or ride on a `pytest.param` inside `pytest.mark.parametrize` to register one scenario per entrypoint. The parametrize may come from a module-level helper whose body is a single `return`, and its parameter list may be a comprehension over a module-level tuple of names — atlan-metabase-app's `entrypoint_matrix(scenario)` is the reference shape. The marker's `rule`, `scenario` and `entrypoint` must resolve to literal strings; a registration the static reader cannot resolve is reported, not counted.

A registration does not define its scenario when the test is skipped or xfail (a decorator, a class decorator, a module or class `pytestmark`, a param-level mark, or an unconditional `pytest.skip()` / `pytest.xfail()` at the top of the body), when it declares `unsupported=True`, when it names a scenario or entrypoint outside the matrix, or when it never calls the required assertion. The assertion may be called from a same-module helper the test calls directly.
