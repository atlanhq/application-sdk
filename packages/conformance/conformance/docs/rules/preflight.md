...
`AppPermissionDeniedError` subclass unless a test on the caught exception selects it or
a classifier receives the exception and the leaf as its default. Classify it first —
`application_sdk.errors.classify_http_exception` for httpx failures, or an `isinstance`
chain — and fall back to a leaf that does not blame the customer (`InternalError` when
the cause is unknown), or narrow the except clause.
