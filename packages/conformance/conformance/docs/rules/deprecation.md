...
- **Interacts with:** ledger-guard is append-only: a recorded type cannot change and an entry cannot be
  ...
  field, so moving off `Any` is required and no longer counts as a break here.
- **Already correct when:** Four shapes are NOT breaks and do not fire: a field whose ledger status is `sunset` (the
  ...
  was WIDENED, an INHERITED field whose base class changed the type, and a move OFF
  `Any`. To retire a field deliberately, mark it `sunset` in contract_schema.lock.json.
