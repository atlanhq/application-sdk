...
- **Already correct when:** Four shapes are NOT breaks and do not fire: a field whose ledger status is `sunset` (the
  ...
  (such as `FilterMap`), is expanded to its target before the comparison — the ledger
  side is not expanded, and an alias declared under `if TYPE_CHECKING:` / `try`, as a
  string, or as a plain `X = <name>` such as `Ident = str` is not recognised, and a
  chain that expands past a size budget is compared unexpanded). To retire a field
