...
- **Compliant example:** atlan-mysql-app app/handler.py — `get_logger(__name__)` at module scope, imported from
  application_sdk.observability.logger_adaptor. No reference app's production code binds
  a logger through logging.getLogger, structlog.get_logger, or loguru's logger.
...
Importing `get_logger` does not exempt a file: a logger bound through
`logging.getLogger()` / `structlog.get_logger()` and then used for log calls is flagged
at the bind line even when the same file imports the adapter.  Configuring a third-party
library's logger inline (`logging.getLogger("httpx").setLevel(...)`, or passing it as a
handler/forwarding target) is not a bind and is not flagged.
