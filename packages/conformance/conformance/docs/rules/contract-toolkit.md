...
`args.metadata.<key>` — is not declared on the corresponding entrypoint's Python `Input`
contract, directly or via any inherited base class or SDK mixin — and the contract does
not set Pydantic `extra='allow'`.
