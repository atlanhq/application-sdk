...
contract, directly or via any inherited base class or SDK mixin — and the contract
neither sets Pydantic `extra='allow'` nor defines a `@model_validator(mode='before')`
that could take the key.
...
* add a `@model_validator(mode='before')` folding the flat keys into `metadata`. Let
nested win over flat, so pre-0.9.0 workflow specs (still persisted and in flight) are
untouched and re-validation stays idempotent; * or set `model_config =
ConfigDict(extra='allow')` when the app genuinely wants to read arbitrary AE-supplied
keys off `model_extra`.
