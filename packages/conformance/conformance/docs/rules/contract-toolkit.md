...
**An artifact that is itself a Pkl module is declared by the module it amends.** When an
entry point takes (or hands off) a `.pkl` file the app evaluates -- a typedef module
amending the typedef toolkit's `Typedefs.pkl`, say -- that module already fixes every
property, so there is no field list to author. Declare it with `PklArtifactSchema` and a
pinned `package://` URI instead:

    artifactSchemas {       ["model_module"] = new PklArtifactSchema {
amendsModule = "package://example.com/models@1.2.3#/Typedefs.pkl"       }     }

This satisfies the rule. An archive (a zip) that bundles such a module is not a
declarable shape: hand off the module itself (FND-2839).
