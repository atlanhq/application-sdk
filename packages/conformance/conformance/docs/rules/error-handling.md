...
- **Interacts with:** The set of forms that actually clear this rule is narrower than it looks, and two of the
  ...
  F005 forbids there. The only log that satisfies both is logger.error with the
  exception routed through a redaction helper (safe_traceback, sanitize_cause_repr); the
  alternative is to return the failure as typed data. Found in a consumer app in
  FND-2569.
