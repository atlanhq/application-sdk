...
`mysql` reference app, whose    generated contract needs no opt-out at all. 2. *Add a
new, bounded field* and mark the old one `deprecated` in    the ledger.  Additions and
status changes are always allowed. 3. *Keep the opt-out with a justified suppression.*
This is the right    answer, not a failure, when the field is an `@entrypoint` contract
field: B005 forbids changing its recorded type and `ledger-guard`    is append-only, so
options 1 and 2 are closed and the carve-out is    genuinely unavoidable.  Say that in
the reason.
