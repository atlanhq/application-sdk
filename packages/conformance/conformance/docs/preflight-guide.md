...
**Contract:** a broad `except` (`Exception`, `BaseException` or bare) in `preflight_check` or a helper it reaches must not build an `AuthError` or `AppPermissionDeniedError` subclass without first looking at what it caught. A fixed leaf there tells the customer every failure in the `try` is a credential or grant problem, including an empty credential, a DNS failure or a source 500.

**Investigate:** list what the `try` body can raise. If only one cause really is a credential or grant failure, the fixed leaf is wrong for the rest. **Fix:** classify the caught exception before choosing the leaf, with `application_sdk.errors.classify_http_exception` for httpx failures or an `isinstance` chain for driver errors, and fall back to a leaf that does not blame the customer for what you cannot name (`InternalError` when the cause is unknown). Narrowing the `except` clause to the exceptions that do mean auth or permission also works. Classifying only the transients is not enough, as a bare `reraise_if_transient(exc)` or as `if transient := is_transient(exc): raise transient`, because every other failure still gets the fixed leaf. Storing a classifier result and then building the fixed leaf anyway does not count either, and neither does handing the classifier an auth or permission leaf as its default, because that default is what every unknown cause gets. **Verify:** rerun `--series F`. F021 disappears, and a test that drives a non-auth failure through the probe asserts the leaf it now reports.
