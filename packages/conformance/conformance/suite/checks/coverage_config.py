...
Discovery
---------
Scans the repo-root ``pyproject.toml`` only. If absent, both rules no-op —
a repo with no coverage config at all is a different (unconfigured, not
misconfigured) concern, out of scope for this check.
...
def discover(root: Path) -> list[Path]:
    ...
    one (there is no other file this series reads).
    """
...
def _is_legit_app_omit(pattern: str) -> bool:
    ...
    if any(marker in norm for marker in _LEGIT_OMIT_MARKERS):
        return True
...
def _t014_message() -> str:
    return (
        ...
        "absent or 0 — coverage is measured and reported but can never fail a run. "
