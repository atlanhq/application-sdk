...
RULES: tuple[RuleDefinition, ...] = (
    RuleDefinition(
        ...
        full_description=(
            ...
            "\n"
            "Files under ``tests/integration/`` are exempt from the construction-\n"
            "call and lifecycle-call violation classes: those harnesses need an\n"
            "in-process worker/client handle to submit workflows and tear down per\n"
            "test, and ``run_dev_combined`` blocks until shutdown with no\n"
            "handle-returning mode to substitute. The removed-v2-import class stays\n"
