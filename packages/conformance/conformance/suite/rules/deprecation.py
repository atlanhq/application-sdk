...
RULES: tuple[RuleDefinition, ...] = (
    RuleDefinition(
        id="B001",
        canonical_reference=(
            "atlan-openapi-app app/connector.py — imports come from the public SDK surface "
            "(application_sdk.app, application_sdk.contracts, application_sdk.errors). "
    ...
    RuleDefinition(
        id="B005",
        canonical_reference=(
            "atlan-metabase-app contract_schema.lock.json — the ledger records each "
            "entrypoint contract field and its type. `atlan-application-sdk-conformance "
            "ledger-guard` enforces it append-only between the base ref and HEAD, so a "
            "removal or a retype is refused there rather than discovered by a customer."
        ),
        rule_interactions=(
            ...
            "on this: it refuses `Any` on a contract field, so replacing `Any` in "
            "place with a concrete type is required and no longer counts as a break "
        ...
        terminal_state=(
            ...
            "including a chain of such aliases, or a public SDK type alias bound by "
            "`from application_sdk… import X` (such as `FilterMap`), is expanded "
        ...
        rationale=(
            ...
            "Customer impact: deployed tenants keep emitting the old payload shape after "
            "the app upgrades under them, so the customer's first run on the new version "
            "fails or silently mis-parses — a break they hit with zero changes on their side."
        ),
        short_description=(
    ...
    RuleDefinition(
        id="B006",
        canonical_reference=(
            "atlan-metabase-app contract_schema.lock.json — one `fields` entry per "
            "entrypoint contract field, regenerated by `atlan-application-sdk-conformance "
            "gen-contract-ledger`. A field missing from the ledger means the ledger was "
            "not regenerated after contract/app.pkl changed, not that the field is wrong."
        ),
        rule_interactions=(
        ...
        rationale=(
            ...
            "Customer impact: an unledgered field is one PR away from B005's customer-facing "
            "break shipping unnoticed — the removal that corrupts deployed tenants' payloads "
            "passes CI clean because the guard had nothing to compare against."
        ),
        short_description=(
        ...
        full_description=(
            ...
            "version-pinned to the checker that raised it — and commit the updated\n"
            "ledger in the same PR as the contract change.  Keep the pin.  CI runs\n"
            "``detect`` from an unpinned ``uvx`` install while a bare ``uv run``\n"
            "resolves the app's *locked* conformance dev dependency, and the\n"
            "generator's output depends on its version: the SDK contract-base field\n"
            "registry it reads grows as the SDK gains fields.  Regenerating with an\n"
            "older locked version therefore rewrites the ledger byte-identically and\n"
