...
RULES: tuple[RuleDefinition, ...] = (
    ...
    RuleDefinition(
        ...
        terminal_state=(
            ...
            "SecretStore protocol rather than os.environ. An inline ignore[S002] is the "
            "correct end state in exactly one case: the SDK offers no seam for the call "
            "being made — e.g. a helper that authenticates only via boto3's default "
            "credential chain, forcing the caller to stage credentials into the "
            "environment. Such a suppression must name the specific SDK function that "
            "lacks an explicit-credentials parameter, so it can be retired when that "
            "seam ships; a suppression that cites only 'platform self-auth' or a "
            "general missing seam is not auditable and is not a terminal state. "
            "Staging into process-global os.environ is also unsafe under concurrency, "
            "so prefer fixing the seam over suppressing (BLDX-1419, FND-2558)."
        ),
        scope=RuleScope.APP,
