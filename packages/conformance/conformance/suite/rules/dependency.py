...
RULES: tuple[RuleDefinition, ...] = (
    ...
    RuleDefinition(
        ...
        short_description=(
            ...
            "non-floating specifier, or missing from uv.lock"
        ),
        full_description=(
            ...
            "per repo; three things are checked, in order:\n"
            "\n"
            ...
            "2. **Specifier can float** — a floor *and* an upper bound, and\n"
            "   not a pin.  ``==0.13.0``, ``===0.13.0``, ``~=0.17.0``, a bare\n"
            "   ``>=0.17.0`` and a bare ``<1.0.0`` are all rejected.\n"
            "3. **Present in** ``uv.lock`` — checked only when a lock exists\n"
