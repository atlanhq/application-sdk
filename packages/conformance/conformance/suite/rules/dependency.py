...
RULES: tuple[RuleDefinition, ...] = (
    ...
    RuleDefinition(
        ...
        rationale=(
            ...
            "package URL in uv.lock to the proxy host. Note that 'uv run' alone is enough "
            "— the rewrite does not wait for a deliberate lock operation, and "
            "'uv lock --upgrade-package X' does not scope it either, because the index "
            "override is machine-level rather than lock-level. The rewrite is silent: it "
            "prints nothing and it touches a file the task at hand never mentioned, so it "
            "is noticed only by someone who happens to read the diff. A project-level "
            "index takes precedence over the machine-wide one and every uv subcommand "
            "reads it, so one stanza closes the whole class. "
            "It can also move a VERSION, which is the part most likely to be missed: the "
            "proxy mirror lags PyPI, so resolving through it can silently DOWNGRADE a "
            "package that was already correctly pinned. Observed on atlan-metabase-app "
            "(FND-2547): a single 'uv run' took atlan-application-sdk-conformance from "
            "0.36.1 back to 0.36.0 while rewriting all 150 sources. A reader who trusts "
            "that only URLs move will not think to check the versions. "
            ...
            "cause is invisible in the diff that produced it."
        ),
        short_description=(
