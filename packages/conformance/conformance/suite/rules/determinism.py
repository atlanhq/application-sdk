...
RULES: tuple[RuleDefinition, ...] = (
    ...
    RuleDefinition(
        ...
        full_description=(
            ...
            "\n"
            "A call that is the direct operand of ``await`` is never flagged: an\n"
            "awaited ``path.read_text()`` is ``anyio.Path`` and an awaited\n"
            "``cursor.fetchall()`` is an async driver.  The name alone cannot\n"
            "separate those from their blocking namesakes; the ``await`` can.\n"
            "\n"
