...
RULES: tuple[RuleDefinition, ...] = (
    ...
    RuleDefinition(
        ...
        rationale=(
            ...
            "coroutine on it. Tree-scale filesystem work (shutil.rmtree / copytree / "
            "move, and the SafeFileOps rmtree / move wrappers) blocks for as long as the "
            "tree takes to walk, which starves a @task's auto-heartbeat and makes "
            "Temporal retry an activity that is still making progress — App."
            "cleanup_files shipped with exactly that bug while this rule's inventory "
            "was network/sleep-only. The same property — duration scales with the "
            "data, not with a fixed syscall cost — covers tree traversal (os.walk / "
            "glob / Path.rglob), pandas and pyarrow readers and writers, whole-file "
            "pathlib accessors, file-handle (de)serialization, and subprocess calls; "
            "an AST sweep of the connector fleet found every one of those blocking a "
            "loop in production code. The correct pattern is to await an async "
            "equivalent, or offload blocking work via App.run_in_thread() inside a "
        ...
        full_description=(
            ...
            "directly, or offload genuinely blocking work with\n"
            "``App.run_in_thread()`` inside a ``@task``.\n"
            "\n"
            "The same data-scale property extends the inventory to tree traversal\n"
            "(``os.walk`` / ``os.scandir`` / ``glob.glob`` / ``Path.glob`` /\n"
            "``Path.rglob``), pandas and pyarrow readers and writers\n"
            "(``pandas.read_sql`` / ``read_parquet`` / ``DataFrame.to_parquet`` /\n"
            "``pq.read_table`` / …), whole-file ``pathlib`` accessors\n"
            ...
            "``urllib.request.Request()`` — so building a session in an\n"
            "``async def`` is silent.  A send on that session is flagged: inline\n"
            "(``requests.Session().get(...)``), or through a name bound from\n"
            "``requests.Session()`` in the same function (``s = ...``, ``s: ... =\n"
            "...``, ``with ... as s``) and then called with ``s.get`` / ``post`` /\n"
            "… / ``request`` / ``send``, or through a ``self.<attr>`` bound to a\n"
            "session in any method of the same class (``__init__`` included).\n"
            "\n"
            "Single-syscall filesystem operations (``os.remove``, ``os.unlink``,\n"
            "``os.rmdir``) are **not** flagged: one inode operation does not earn a\n"
            "thread hop, and flagging them would bury the tree-scale findings.  The\n"
            "in-memory string forms ``json.loads`` / ``json.dumps`` are likewise out\n"
            "of scope — CPU, not I/O, and used for small payloads everywhere.  Only\n"
            "(de)serializers with a ``load``/``loads`` split are matched, so PyYAML\n"
            "and ``csv`` are absent: ``yaml.safe_load`` and ``csv.reader`` take a\n"
            "string as readily as a handle, and matching them by name would flag the\n"
            "very in-memory parsing ``json.loads`` is allowed to do.\n"
            "\n"
            "Across that data-scale inventory, a call the source already marks as\n"
            "async is never flagged — the operand of ``await``, and the iterable of\n"
            "an ``async for``.  An awaited ``path.read_text()`` is ``anyio.Path``;\n"
            "``async for p in path.glob(...)`` is an async iterator.  The name alone\n"
            "cannot separate those from their blocking namesakes; the async syntax\n"
            "can.  A ``lambda``\n"
            "body counts as a sync scope, like a nested ``def``, so the prescribed\n"
            "offload — ``await run_in_thread(lambda: list(path.rglob('*')))`` — is\n"
            "silent rather than a finding in its own right.\n"
            "\n"
