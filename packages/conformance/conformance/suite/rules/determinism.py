...
RULES: tuple[RuleDefinition, ...] = (
    ...
    RuleDefinition(
        ...
        full_description=(
            ...
            "async is never flagged — the operand of ``await``, the iterable of an\n"
            "``async for``, the context expression of an ``async with``.  An awaited\n"
            "``path.read_text()`` is ``anyio.Path``; ``async for p in\n"
            "path.glob(...)`` is an async iterator.  The name alone cannot separate\n"
            "those from their blocking namesakes; the async syntax can.  A ``lambda``\n"
