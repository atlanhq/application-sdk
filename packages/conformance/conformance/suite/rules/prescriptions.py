...
RULES: tuple[RuleDefinition, ...] = (
    ...
    RuleDefinition(
        ...
        full_description=(
            ...
            "  module bound to it).\n"
            "\n"
            "Names resolve by lexical scope, as Python binds them: a parameter or\n"
            "local helper that shadows an imported encoder is not flagged, and an\n"
            "import inside one function does not reach another.  A simple saved\n"
            "alias is followed (``encode = asset.to_nested_bytes; encode()``,\n"
            "``enc = to_atlas_format``); ``getattr`` / ``functools.partial`` /\n"
            "container indirection is out of scope.\n"
            "\n"
