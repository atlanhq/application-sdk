...
RULES: tuple[RuleDefinition, ...] = (
    RuleDefinition(
        ...
        full_description=(
            ...
            ' a ``workflow_type`` of the form ``"<app>:<wire-name>"`` in the'
            " manifest DAG (the BLDX-1342 route/card split: one marketplace card plus"
            " additional DAG-invoked routes, without per-entry-point bundle subdirs)."
            " A code ``@entrypoint`` the DAG never routes to is flagged. When the"
            " manifest declares no such routes (legacy), the check falls back to"
            " permitting at most one ``@entrypoint``.\n"
            "\n"
            "* **Legacy workflow type aliases (CNCT-199, CONNECT-1081):** an app may"
            " declare inbound-only Temporal types that external callers still"
            " dispatch. The source read here is the generated manifest's"
            " ``legacy_workflow_types`` block, not the ``App`` class attribute —"
            " K015 is what holds those two in agreement. A bare (colon-free) DAG node"
            " whose ``workflow_type`` equals a declared alias routes the alias's"
            " target entry point, provided the node's own identity does not"
            " contradict the declaration: a node whose ``app_name`` names an app this"
            " repo does not define dispatches on that app's worker, so it routes"
            " nothing here even when its type is declared locally. A node carrying no"
            " ``app_name`` routes on the declaration alone (task queues play no part)."
            " A bare node **not** declared as an alias is a platform/other-app node"
            " (such as ``PublishWorkflow``) and routes nothing.\n"
            "\n"
            "* **Non-literal ``name=``:** a ``@entrypoint(name=variable)`` cannot be"
            " statically resolved and is flagged as unverifiable.\n"
            "\n"
