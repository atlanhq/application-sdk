...
RULES: tuple[RuleDefinition, ...] = (
    RuleDefinition(
        ...
        full_description=(
            ...
            "\n"
            "* **``workflow_type`` overrides (CNCT-199):** an ``@entrypoint`` that pins"
            " ``workflow_type=`` is routed by that verbatim string, not by a"
            ' colon-qualified ``"<app>:<wire>"`` route. A bare (colon-free) DAG node'
            " whose ``workflow_type`` equals the override counts as a route **only"
            " when it is provably local**: its ``app_name`` equals the entry point's"
            " app, or — when the node carries no ``app_name`` at all — its"
            " ``task_queue`` matches a queue this app's own routes use. A bare node"
            " that names a *different* app is foreign even on a shared queue and does"
            " not route the override (a same-named foreign platform node such as"
