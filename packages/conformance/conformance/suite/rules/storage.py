...
RULES: tuple[RuleDefinition, ...] = (
    RuleDefinition(
        ...
        since="0.6.0",
        rationale=(
            "App.upload/App.download/App.upload_refs are themselves @task methods. "
            "Calling self.upload(...)/self.download(...)/self.upload_refs(...) "
            "inside another @task-decorated "
        ...
        full_description=(
            "An ``App`` subclass calls ``self.upload(...)``, ``self.download(...)``\n"
            "or ``self.upload_refs(...)``\n"
            "from within a ``@task``-decorated method.  ``App.upload``,\n"
            "``App.download`` and ``App.upload_refs`` are themselves ``@task``\n"
            "methods, so this nests an\n"
            ...
            "store — see BLDX-1398.\n"
            "\n"
            "The method set is the framework tasks that move bytes across the store\n"
            "boundary.  ``App`` has other framework tasks — ``verify_refs``,\n"
            "``cleanup_files``, ``cleanup_storage`` — and nesting one of those is\n"
            "the same task-within-a-task error, but it is not a transfer, so it is\n"
            "deliberately out of scope for this id rather than silently folded into\n"
            "a rule whose name and remediation are about the transfer seam.\n"
            "\n"
    ...
    RuleDefinition(
        ...
        full_description=(
            ...
            "\n"
            "Where the prefix call was moving a *whole tree* a fanned-out step\n"
            "produced, ``App.upload_refs()`` (SDK 3.33.2) is the hoist that keeps\n"
            "the shape: it takes the ``FileReference`` declaration and lands every\n"
            "entry under one destination prefix, by reference rather than by\n"
            "scanning a directory — which is what makes it correct on a distributed\n"
            "run, where the calling pod holds only the subset of files it wrote\n"
            "itself.\n"
            "\n"
