...
RULES: tuple[RuleDefinition, ...] = (
    ...
    RuleDefinition(
        ...
        rationale=(
            ...
            "calls self.upload() — or its peer framework task self.upload_refs(), "
            "which delivers a FileReference declaration through the same upload "
            "body — the ENABLE_ATLAN_UPLOAD path is unreachable: "
        ...
        short_description=(
            "SDR app has no self.upload()/self.upload_refs() call in source — "
            "ENABLE_ATLAN_UPLOAD path unreachable"
        ),
        full_description=(
            "For apps declaring ``self_deployed_runtime: true`` in ``atlan.yaml``,\n"
            "at least one Python source file (outside ``tests/``) must contain a\n"
            "``self.upload(`` or ``self.upload_refs(`` call.\n"
            "\n"
            ...
            "runs in a local-only mode.  If neither call appears\n"
            ...
            "\n"
            "**``App.upload_refs()`` satisfies this rule** (SDK 3.33.2 and later).\n"
            "It is not a wrapper around ``upload`` but a peer framework task over\n"
            "the identical ``_upload_impl`` body: same ``ENABLE_ATLAN_UPLOAD``\n"
            "gate, same ADR-0014 dual-write routing, same transformed-asset\n"
            "validation — and it additionally verifies the delivered tree against\n"
            "the declaration in the upstream store before returning.  It is the\n"
            "*stronger* hand-off for a fanned-out connector, which is the shape\n"
            "that matters here: a transfer that scans ``<output>/transformed`` from\n"
            "the calling pod sees only the subset of files that pod happened to\n"
            "write, and on a fully distributed run sees nothing at all.  Treating\n"
            "it as an absence would have penalised exactly the migration this rule\n"
            "wants.\n"
            "\n"
            ...
            "real call.  Fix by adding ``await self.upload(...)`` — or\n"
            "``await self.upload_refs(...)`` where the hand-off is a declaration\n"
            "rather than one directory — to the ``run()`` method or the relevant\n"
            "``@entrypoint`` method.\n"
            "\n"
            ...
            "visible where the checker can see it (call ``self.upload(...)``,\n"
            "``self.upload_refs(...)``, or ``super().upload(...)``, or keep the\n"
            "delegated helper in the same\n"
            ...
            "invisible.\n"
            "\n"
            "Note: P008 flags ``self.upload()``/``self.upload_refs()`` *inside*\n"
            "``@task`` methods (the\n"
            ...
            "of an upload call is by design, not a gap.\n"
        ),
        help_uri=(
            "https://github.com/atlanhq/application-sdk/blob/main/"
    ...
    RuleDefinition(
        ...
        rationale=(
            ...
            "upload_to_atlan method, with no self.upload() / self.upload_refs() "
            "anywhere, has "
        ...
        full_description=(
            ...
            "same-class delegation) and neither ``self.upload(`` nor\n"
            "``self.upload_refs(`` is called anywhere in\n"
            "the app source.\n"
            "\n"
            ...
            "``FileReference`` declaration a fanned-out step produced, and\n"
            "``await self.upload(...)`` otherwise — in the ``run()`` method or the\n"
            "relevant ``@entrypoint`` method (crawler AND miner) — and delete the\n"
            "bridge.  The choice matters on exactly the connectors this rule finds:\n"
            "a bridge that scans one directory is usually there *because* the\n"
            "transforms fanned out, and swapping it for a ``self.upload(local_path)``\n"
            "over the same directory greens a single-container e2e while uploading\n"
            "only the subset of files the calling pod happened to write.\n"
            "``upload_refs`` streams from the deployment store for the files this pod\n"
            "never held.  If the bridge exists because neither call can express\n"
