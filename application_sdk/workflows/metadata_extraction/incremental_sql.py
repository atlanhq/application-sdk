...
The workflow implements a 4-phase execution:
1. Setup: get_workflow_args → fetch_incremental_marker → read_current_state
2. Base Extraction: fetch_databases → fetch_schemas → fetch_tables → transform
...
Usage:
    ...
    @workflow.defn
    class OracleWorkflow(IncrementalSQLMetadataExtractionWorkflow):
        activities_cls = OracleActivities
        
        # No run() override needed - everything inherited!
"""
...
class IncrementalSQLMetadataExtractionWorkflow(BaseSQLMetadataExtractionWorkflow):
    ...
    async def _run_incremental_column_extraction(
        ...
        if total_records > 0:
            ...
            if not transform_args.get("connection", {}).get("connection_qualified_name"):
                raise ValueError(
