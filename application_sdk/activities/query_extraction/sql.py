...
class SQLQueryExtractionActivities(ActivitiesInterface):
    ...
    async def write_marker(
        ...
        with safe_open(marker_file_path, "w") as f:
            f.write(last_marker)
    ...
    async def read_marker(self, workflow_args: Dict[str, Any]) -> Optional[int]:
        ...
        try:
            ...
            if not safe_exists(marker_file_path):
                logger.warning(f"Marker file does not exist at {marker_file_path}")
                return None
            with safe_open(marker_file_path, "r") as f:
    ...
    async def get_query_batches(
        ...
        safe_makedirs(os.path.dirname(metadata_file_path), exist_ok=True)
        with safe_open(metadata_file_path, "w") as f:
