...
Lives under ``application_sdk.common`` because both transformation paths
need it: the v2 ``AtlasTransformer`` and the v3 asset-mapper seam
(:mod:`application_sdk.common.asset_serialization`).  Its original home,
``application_sdk.transformers.common.last_sync``, is inside a package
that raises a ``DeprecationWarning`` on import and is removed in v4.0 —
so a v3 seam could not import it there without warning on every SDK
import.  That path still re-exports these names and keeps working until
v4.0 (FND-2097).
