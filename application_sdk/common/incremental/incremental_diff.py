...
def create_incremental_diff(
    ...
    copy_workers: int = 3,
    get_backfill_tables_fn: Optional[Callable[[Path, Optional[Path]], Optional[Set[str]]]] = None,
) -> IncrementalDiffResult:
    ...
    if get_backfill_tables_fn is not None:
        backfill_tables = get_backfill_tables_fn(
            transformed_dir, previous_state_dir
        ) or set()
