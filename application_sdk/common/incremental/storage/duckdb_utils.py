...
Provides:
- DuckDBConnectionManager: File-backed connection with auto cleanup (context manager)
- managed_duckdb_connection: For optional connection reuse in helper functions
- SQL helpers: escape_sql_string, json_scan, get_parent_table_qn_expr

Usage:
    # File-backed connection with auto cleanup
    with DuckDBConnectionManager() as conn_manager:
        conn_manager.connection.execute("SELECT ...")

    # Reuse existing connection (in helper functions)
    with managed_duckdb_connection(existing_conn) as conn:
        conn.execute("...")
"""
...
try:
    ...
    DuckDBConnection = Any  # type: ignore[misc, assignment]
