...
class AtlanLoggerAdapter:
    def __init__(self, logger_name: str) -> None:
        ...
        if ENABLE_OTLP_LOGS:
            try:
                ...
                self.logger.bind(logger_name=self.logger_name).error(
                    f"Failed to setup OTLP logging: {str(e)}"
                )
    ...
    def _parse_otel_resource_attributes(self, env_var: str) -> dict[str, str]:
        try:
            ...
            self.logger.bind(logger_name=self.logger_name).error(
                f"Failed to parse OTLP resource attributes: {str(e)}"
            )
        return {}
    ...
    def otlp_sink(self, message: Any):
        ...
        try:
            ...
            self.logger.bind(logger_name=self.logger_name).error(
                f"Error processing log record: {e}"
            )
