...
def _record_proxy_failure(
    ...
    *log_args: object,
    ...
    if level == "debug":
        ...
        logger.info(log_message, *log_args, exc_info=exc_info)
    else:
    ...
    async def prometheus_metrics() -> Response:
        ...
        if enable_temporal_core_metrics and temporal_bind_address:
            ...
            try:
                ...
                _record_proxy_failure(
                    type(exc).__name__,
                    "Temporal core metrics endpoint unavailable at %s; serving /metrics without Temporal core series",
                    temporal_metrics_url,
                    level="debug",
                )
            except httpx.RequestError as exc:
                # Non-connect request failures are unexpected; keep the traceback for debugging.
                _record_proxy_failure(
                    type(exc).__name__,
