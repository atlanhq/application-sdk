...
from application_sdk.credentials.credentials_utils import (
    apply_secret_values,
    process_secret_data,
)
...
class AWSSecretsManagerCredentialProvider(CredentialProvider):
    ...
    async def get_credentials(
        ...
        try:
            ...
            logger.error(
                f"Error fetching credentials from AWS Secrets Manager: {str(e)}"
            )
            logger.error(f"Traceback: {traceback.format_exc()}")
            raise CredentialError(
                f"Failed to fetch credentials from AWS Secrets Manager: {str(e)}"
            )
    ...
    def _validate_params(self, source_credentials: Dict[str, Any]) -> Dict[str, str]:
        ...
        if not params["aws_secret_region"]:
            raise CredentialError(
                "aws_secret_region is required for AWS Secrets Manager"
            )

        return params

    async def _fetch_secret(
        self, aws_secret_arn: str, aws_secret_region: str, secret_store: str
    ) -> Dict[str, Any]:
        """
        ...
        with DaprClient() as client:
            secret = client.get_secret(
                store_name=secret_store_region, key=aws_secret_arn
            )
