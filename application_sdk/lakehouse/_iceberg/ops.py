...
def scan_records(
    ...
    *,
    ...
    table = catalog.load_table(_catalog.identifier(namespace, table_name))
    scan = table.scan(
...
def ensure_table(
    ...
    identifier = _catalog.identifier(namespace, table_name)
    try:
        return catalog.load_table(identifier)
    except (NoSuchTableError, Exception):
        _catalog.ensure_namespace(catalog, namespace)
...
def table_exists(catalog: Catalog, namespace: str, table_name: str) -> bool:
    try:
        catalog.load_table(_catalog.identifier(namespace, table_name))
        return True
    except Exception:
        return False
