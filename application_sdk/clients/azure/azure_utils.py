...
def parse_azure_url(url: str) -> Dict[str, str]:
    ...
    try:
        ...
        if not parsed.scheme or not parsed.netloc:
            raise CommonError(
                f"{CommonError.CREDENTIALS_PARSE_ERROR}: Invalid URL format"
            )
...
def validate_azure_credentials(
    ...
    try:
        ...
        if auth_type != "service_principal":
            return (
                False,
                f"Only 'service_principal' authentication is supported. Received: {auth_type}",
            )
    ...
    **kwargs: Any,
    ...
    try:
        if service_type == "blob":
            ...
            raise CommonError(
                f"{CommonError.CREDENTIALS_PARSE_ERROR}: Unsupported service type: {service_type}"
            )
...
def extract_azure_resource_info(resource_id: str) -> Dict[str, str]:
    ...
    try:
        ...
        if len(parts) < 6:
            raise CommonError(
                f"{CommonError.CREDENTIALS_PARSE_ERROR}: Invalid resource ID format"
            )
