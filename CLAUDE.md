...
**IMPORTANT**: In GitHub Actions, do not inline shell that contains conditional logic (`if`/`else`, loops, `case`). Extract it into a script under `.github/scripts/` (Python, with a pytest in `.github/scripts/tests/`, run by the `CI script tests` workflow) and have the workflow `run:` call that script. Keep inlined `run:` blocks to straight-line commands. Rationale: branching logic in YAML can't be regression-tested. See `docs/standards/ci.md`.
...
- `docs/agents/canonical-apps.md` — the three public reference apps (openapi, mysql, metabase). Read one of these for how a consumer app *should* look; do not generalise from an arbitrary `atlan-*-app`, which may be mid-migration or carry deprecated patterns.
...
- `docs/standards/symbols.md` — process for removing/renaming a Python symbol: you may not remove a name that was not already marked deprecated in the previous release. Checked by the Symbol Removal Check workflow (advisory until added to the `main` ruleset). Read it before deleting or renaming anything importable.
- `docs/standards/cross-repo-contracts.md` — values this SDK produces that other repos read (e.g. the served manifest's resolved `task_queue`); read the entry before changing one.
- `docs/standards/ci.md` — CI authoring: no inlined conditional shell; put logic in tested `.github/scripts/` scripts.
