...
server_release.py
----------------------
Computes the next semver for atlan-application-sdk-server, updates
packages/server/pyproject.toml and server_sdk/__init__.py, and
prepends a new section to packages/server/CHANGELOG.md.

Usage:
    python server_release.py

Exits 0 with "skip=true" written to GITHUB_OUTPUT when there are no
unreleased commits touching packages/server/** (lock files excluded — see
...
PYPROJECT = "packages/server/pyproject.toml"
VERSION_PY = "packages/server/server_sdk/__init__.py"
CHANGELOG = "packages/server/CHANGELOG.md"
RELEASE_NOTES_FILE = "/tmp/server-release-notes.md"
TAG_PREFIX = "server-v"
PACKAGE_DIR = "packages/server"
...
# an "unreleased server commit" and trigger a version bump. Excluding them by
...
# packages/server/pyproject.toml, which stays in scope.
...
def main():
    ...
    if not tag_exists(tag):
        # No tag for the current version. Check whether any server tag
        ...
        if existing.strip():
            sys.exit(
                f"ERROR: tag {tag!r} not found but other server tags exist. "
    ...
    if not subjects.strip():
        print(f"No unreleased server commits (current: {current}).")
