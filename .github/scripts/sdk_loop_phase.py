...
def review_prompt(
    ...
    parts = [
        ...
        "",
        "`.mothership/` is a DOT-directory, and your Glob and Grep tools are",
        "ripgrep-backed, which skips hidden paths by default. Globbing anything",
        "under it returns 0 matches even though the files are right there — a",
        "live round saw exactly that for `.mothership/pr-review/agents/*.md`.",
        "",
        "So read those files by EXPLICIT PATH with your file-read tool, or list",
        "them with `bash ls`. Never conclude from an empty glob that a playbook",
        "asset is missing; check with `ls` before believing it.",
        "",
