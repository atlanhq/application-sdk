...
# phase runs on kimi-k3; the resolve phase on gpt-5.6-luna via opencode, both
...
#                                     to run this lane. UNSET MEANS NOBODY: a
#                                     lane that pushes on its own should be shut
#                                     when it is unconfigured, not open.
    ...
    steps:
      ...
      - name: Authorize, dismiss duplicates, pin the baseline
        ...
        env:
          ...
          # The allowlist covers workflow_dispatch too. Without that it is a
          # one-click bypass: anyone with write access could dispatch the lane
          # and skip the comment gate entirely.
          ACTOR: ${{ github.event.comment.user.login || github.actor }}
          ALLOWED_ACTORS: ${{ vars.SDK_LOOP_ALLOWED }}
