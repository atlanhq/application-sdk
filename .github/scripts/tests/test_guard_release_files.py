...
class TestViolationsOffBumpBranch:
    @pytest.mark.parametrize(
        ...
        [
            ...
            # server_release.py exits with "tag not found but other server tags
            # exist" when the declared version has no matching server-v* tag --
            # the conformance failure mode this guard was written for -- so the
            # server package has to be guarded the same way.
