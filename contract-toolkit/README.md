...
- [`examples/scheduled/`](examples/scheduled/) — cron background job via `schedules`; renders `triggers.schedules` into `manifest.json` (multiple schedules, non-UTC timezone, a `PAUSED` one). See [Schedules](docs/reference.md#schedules-background-jobs). Also carries `events` — including two `streamingEnabled` triggers (real-time and batched) that dispatch through AE's streaming shard instead of a per-batch run. See [Streaming Dispatch](docs/reference.md#streaming-dispatch-on-event-triggers).
