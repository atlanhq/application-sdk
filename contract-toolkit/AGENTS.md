...
Releases are automated and require no manual version input:

1. **Auto-trigger (Phase A):** When any non-release PR that touches
   `contract-toolkit/**` (or related SDK paths) is merged to `main`,
   `Release Contract Toolkit` fires automatically. It uses
   `git cliff --bumped-version` — scoped to `contract-toolkit/**` paths and
   `contract-toolkit-v*` tags — to compute the next semver from unreleased
   conventional commits. If there is nothing to release the workflow exits
   silently. Otherwise it bumps `contract-toolkit/src/PklProject`, regenerates
   `contract-toolkit/CHANGELOG.md`, sanity-builds the PKL package, and
   force-pushes to the fixed `bump-version-contract-toolkit` branch. If no open
   PR exists for that branch, one is created labeled `release:contract-toolkit`.
   If a PR already exists, the force-push updates it in place.
2. **Review and merge the release PR.** The PR runs the standard Contract Toolkit
...
3. **Auto-publish (Phase B):** Merging the labeled PR fires `Publish Contract
   ...
   GitHub Pages build, and tags `contract-toolkit-v<version>` at the merge commit.

The `workflow_dispatch` trigger on Phase A is retained as an escape hatch to
force a re-check or re-create the release PR without waiting for a feature merge.
Commit scoping: `--include-path` ensures SDK-only commits are invisible to the
contract-toolkit version bump, and vice versa.
