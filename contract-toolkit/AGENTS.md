...
Releases are a two-step manual flow from the `atlanhq/application-sdk` repository:

1. Actions -> Release Contract Toolkit -> Run workflow -> enter semver version.
   This bumps `contract-toolkit/src/PklProject`, regenerates
   `contract-toolkit/CHANGELOG.md`, sanity-builds the PKL package, pushes the
   changes to `release/contract-toolkit-v<version>`, and opens a PR labeled
   `release:contract-toolkit` against `main`. The PR runs the standard
   Contract Toolkit Validation checks and needs a codeowner review.
2. Merge the release PR. Publish Contract Toolkit then fires automatically:
   it builds the PKL package from merged `main`, publishes the artifacts under
   `app-contract-toolkit/` on the `gh-pages` branch, triggers a GitHub Pages
   build, tags `contract-toolkit-v<version>` at the merge commit, and creates
   the GitHub Release with the rendered notes and package assets.
