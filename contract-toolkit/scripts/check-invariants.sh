...
# --------------------------------------------------------------------------
# 1. _input.py: dict[str, Any] requires allow_unbounded_fields=True
# --------------------------------------------------------------------------
echo ":: Checking allow_unbounded_fields invariant..."
for f in $(find examples -name '_input.py'); do
  if grep -q 'dict\[str, Any\]' "$f" && ! grep -q 'allow_unbounded_fields=True' "$f"; then
    echo "FAIL: $f has dict[str, Any] but missing allow_unbounded_fields=True"
    fail=1
  fi
done
