...
| [application_lakehouse_events.py](./application_lakehouse_events.py) | AE-triggered event processing using `application_sdk.lakehouse.events_read` + `events_ack`. Reads pending events from the lakehouse in batches, dispatches to a `handler`, publishes the AE Parquet ack. Requires the `[lakehouse]` install extra. |
