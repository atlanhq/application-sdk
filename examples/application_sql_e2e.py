...
async def application_sql(daemon: bool = True) -> Dict[str, Any]:
    ...
    utilities_worker = UtilitiesAtlanWorker(utilities_client)
    await utilities_worker.start(daemon)

    publish_client = TemporalClient(application_name=AtlasPublishAtlanWorker.TASK_QUEUE)
