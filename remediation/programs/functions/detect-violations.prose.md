...
PYTHONPATH=conformance uv run python -m suite.runner \
  --repo <scope> \
  --series <series> \
...
with open("remediation/runs/detect.sarif") as f:
    raw = json.load(f)
report = suite.schema.sarif.SarifReport.model_validate(raw)
```
...
`suite.schema.disposition`.  Keep only results whose disposition matches the
