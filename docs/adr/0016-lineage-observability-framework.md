...
1. **(A) The framework lives centrally in the SDK**, at `application_sdk/observability/lineage/`, as the single stable import surface for every connector repo and the publish-app. **Taxonomy ownership is split:** the SDK owns the **top-level codes** (the stable `ReasonCategory` enum) plus the generic machinery (`ReasonCode`, `ReasonCodeRegistry`, `get_reason_category`), but ships **no concrete subcode registry**. Granular **subcodes** are connector/domain-specific behavior and live in their owning repo (a connector's `app/lineage/reasons.py`; the publish-app's ARS registry), each subcode mapped to exactly one SDK category. This keeps the SDK free of connector-specific variables/statuses/behavior and lets a taxonomy change ship without an SDK release.
2. **(B) The in-activity tracker engine is a verbatim port of the argo `LineageObservabilityTracker` / `NoOp` / category taxonomy + `ReasonCode` machinery** — `build_output` is byte-stable, locked by the argo tracker test suite ported as a regression gate. (The argo `TABLEAU_REASON_CODES` subcode dict is NOT shipped in the SDK; per (A) it is relocated to the connector repo.) The only changes are severing the `marketplace_scripts` import roots and two additive methods (`emit_intent`, `success_keys`) that touch new state only.
