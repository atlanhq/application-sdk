...
| `ATLAN_QUARANTINE_PREFIX` | `quarantine` | Top-level object-store prefix for data marked `quarantined=True` (raw content downloaded from a source system). The tier's ordinary prefix becomes a sub-prefix beneath it, so lifecycle still applies — see [ADR-0021](adr/0021-quarantined-storage.md). Override to align the root with a deployment's own bucket policy; an empty value falls back to the default rather than collapsing quarantined paths onto ordinary ones. **Note:** Atlan's blob-storage proxy allowlists which top-level prefixes may be written upstream, so the quarantine root must be added there before quarantined writes can reach the upstream store. |
