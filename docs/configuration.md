...
| Variable | Default | Description |
|----------|---------|-------------|
| `ATLAN_WORKER_MAX_CONSECUTIVE_RESTARTS` | `10` | Soft threshold: after this many consecutive worker-fatal failures the supervisor escalates log severity to ERROR (so a genuinely stuck worker is loud in monitoring) but **keeps recovering** — it never exits, because this failure class is fixed by a restart. The counter resets after a healthy run window. |
