...
| `TEMPORAL_MAX_CONCURRENT_ACTIVITIES` | `100` | Fixed number of activity slots per worker. Only used when `ATLAN_WORKER_TUNER_MODE=fixed`. |
| `ATLAN_WORKER_TUNER_MODE` | `fixed` | Worker slot tuning mode: `fixed` (static slot counts) or `resource` (Temporal's resource-based tuner sizes slot handout from observed CPU/memory; the fixed `max_concurrent_*` limits are not passed to the worker). Unrecognized values warn and fall back to `fixed`. See [ADR-0016](adr/0016-resource-based-worker-tuning.md). |
| `ATLAN_WORKER_TUNER_TARGET_MEMORY` | `0.8` | Target fraction `(0, 1]` of system memory the resource-based tuner aims to keep in use. Only used when `ATLAN_WORKER_TUNER_MODE=resource`. Invalid or out-of-range values warn and fall back to the default. |
| `ATLAN_WORKER_TUNER_TARGET_CPU` | `0.9` | Target fraction `(0, 1]` of system CPU the resource-based tuner aims to keep in use. Only used when `ATLAN_WORKER_TUNER_MODE=resource`. Invalid or out-of-range values warn and fall back to the default. |
