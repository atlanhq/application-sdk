...
| `UPSTREAM_OBJECT_STORE_NAME` | `atlan-objectstore` | Dapr object store for uploading data to the Atlan platform (SDR deployments only). If the named component is absent, `upstream_storage` is `None` and `App.upload()`/`App.download()` fall back to the deployment store. If the name *equals* `DEPLOYMENT_OBJECT_STORE_NAME` (in-cluster charts point both at one component), `upstream_storage` is the deployment store itself — one write, no self-copy — and `ENABLE_ATLAN_UPLOAD` logs a `WARNING` because there is no boundary to hand off across. Ask `context.single_store`, not `upstream_storage is None`, for the topology. See [ADR-0014](adr/0014-two-store-storage-architecture.md). |
| `EVENT_STORE_NAME` | `eventstore` | Dapr pub/sub component name. |
| `ATLAN_EVENT_INGRESS_DIRECT_FALLBACK` | `true` | When the Dapr `EVENT_STORE_NAME` binding fails in a way that proves the event got no response (connection, TLS or DNS failure, missing binding), publish the lifecycle event (`worker_start`, `token_refresh`, workflow/activity start/end) directly to Event Ingress over HTTPS from the app process, reusing the same payload and bearer token and the `url` of the `bindings.http` component on disk. Errors that may already have been delivered (an upstream HTTP status) are not re-sent. Backs off 60 s after a failed direct publish. Logged at WARNING as `FALLBACK ACTIVE` / `Published event via direct HTTPS fallback`, `FALLBACK SKIPPED` / `FALLBACK SUSPENDED` when not attempted, `FALLBACK FAILED` / `FALLBACK UNAVAILABLE` at ERROR. Set `false` to disable. |
...
| `ATLAN_STORAGE_LOCK_WAIT_PROGRESS_SECONDS` | `30` | Interval (seconds) between progress marks while an activity waits on a per-destination storage lock (a second download of the same path queues behind the first). Capped at a quarter of `ATLAN_MAX_NO_PROGRESS_SECONDS` so a queued activity is never killed as stalled; floored at `1`. |
| `ATLAN_STORAGE_VERIFY_TRANSFERS` | `true` | Kill-switch for transfer integrity validation. When enabled, every upload confirms the local file did not shrink while it was read and that the store recorded the bytes that were sent, and every download confirms it wrote as many bytes as the store declared and — when a `{key}.sha256` sidecar exists — that they hash to the digest the producer recorded. A content mismatch raises the non-retryable `StorageIntegrityError`. Set to `false` for a backend whose HEAD-after-write is not read-your-writes consistent. |
| `ATLAN_STORAGE_WRITE_SIDECARS` | `true` | Kill-switch for `{key}.sha256` sidecar emission. Sidecars are what let a *downstream* app detect an artifact its producer truncated. Set to `false` for a deployment whose object-store consumers cannot tolerate the extra keys (e.g. a reader that globs a prefix without filtering sidecars). Verification is independent — downloads still verify whatever sidecars they find. |
