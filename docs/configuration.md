...
| `UPSTREAM_OBJECT_STORE_NAME` | `atlan-objectstore` | Dapr object store for uploading data to the Atlan platform (SDR deployments only). If the named component is absent, or the name equals `DEPLOYMENT_OBJECT_STORE_NAME` (in-cluster charts point both at one component), `upstream_storage` is `None` and `App.upload()`/`App.download()` use the deployment store — including under `ENABLE_ATLAN_UPLOAD`. See [ADR-0014](adr/0014-two-store-storage-architecture.md). |
