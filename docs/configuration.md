...
| `ATLAN_STORAGE_RESUME_DOWNLOADS` | `true` | Kill-switch for resumable chunked downloads. When enabled, an interrupted chunked download keeps its in-flight part file (`.sdk-partial/{name}.part` beside the destination) plus a `{path}.transfer-state` checkpoint sidecar, and a retry fetches only the missing ranges. Set to `false` to restore delete-partial-on-failure behaviour. |
