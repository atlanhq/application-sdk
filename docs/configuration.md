...
| Variable | Default | Description |
|----------|---------|-------------|
| `ATLAN_WORKER_MAX_CONSECUTIVE_RESTARTS` | `10` | Behaviour past this many consecutive failures depends on the failure class. The recoverable class (a transient Temporal frontend auth rejection — identified by the gRPC `PermissionDenied` status name or an explicit JWKS mention — and the watchdog's `WorkerPollStalledError`) is fixed by a restart, so the supervisor only escalates log severity to ERROR and **keeps recovering** — it never exits. A failure outside that class (bad credentials, a misconfigured task queue) is not fixed by a restart, so past this threshold the supervisor **re-raises and exits** — a permanent misconfiguration still fails loud rather than presenting as a ready, alive pod that never polls. The generic `unauthorized` detail text alone is *not* treated as recoverable, so a steady bad-credential 401/403 re-raises as intended. The counter resets after a healthy run window. |
...
| `ATLAN_WORKER_POLL_LIVENESS_GRACE_SECONDS` | `90` | Poll-liveness watchdog: seconds a (re)built worker is given to register as an active poller, and the sustained window of definitive poller-absence before the worker is declared a dead "zombie" poll loop and rebuilt. Transient frontend describe failures (e.g. the auth-skew window itself) never trip it. |
| `ATLAN_WORKER_POLL_LIVENESS_INTERVAL_SECONDS` | `30` | Poll-liveness watchdog: how often (seconds) to probe the Temporal frontend for this worker's identity in the task-queue pollers. |
