...
## Temporal / Workflow Connectivity

Temporal connectivity is managed automatically by `application_sdk.main` via CLI modes (`--mode handler` / `--mode worker`). App developers do not interact with Temporal clients directly. See [Entry Points](../guides/architecture.md) for details.

## Summary

The `clients` module abstracts interactions with external services.

`SQLClient` subclasses (configured via `DB_CONFIG`) provide the database engine and query execution methods, which are used by activities to fetch data.
