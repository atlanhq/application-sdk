...
- [ ] **`@task` only** — no `@workflow.defn`, `@activity.defn`, `@auto_heartbeater` decorators. No `workflow.execute_activity_method()` calls. The `from temporalio import workflow` import is permitted **only** for `@workflow.signal` / `@workflow.query` / `@workflow.update` handler methods on the App subclass (BLDX-1283 — the SDK lifts these onto the generated workflow class). No `from temporalio import activity` in app code.
