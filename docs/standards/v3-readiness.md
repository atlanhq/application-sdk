...
- [ ] **`@task` only** — no `@workflow.defn`, `@activity.defn`, `@auto_heartbeater` decorators. No `workflow.execute_activity_method()` calls. Runtime interactions (`@signal`, `@query`, `@update`) must be imported from `application_sdk.app`, not from `temporalio.workflow` directly (BLDX-1283). No `from temporalio import activity` in app code.
...
The SDK generates workflow/credential/manifest/input artifacts from a single `contract/app.pkl`. See [`.claude/skills/make-contract/SKILL.md`](../../.claude/skills/make-contract/SKILL.md).
...
There is no one-click runner: the `v3-readiness-check.yaml` workflow that once wrapped §5.1 — §5.2 was removed in FND-2661, having gone unrun since 2026-05-04 with no caller in any repo. Run the commands below against a checkout of the app instead.
