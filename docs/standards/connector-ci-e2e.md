...
* with `temporal_address` / `E2E_TEMPORAL_ADDRESS` set, the harness reads `DescribeTaskQueue` for each still-`Running` node's own queue at the stop point and says which it was — `NO poller has claimed it: Temporal reports 0 pollers on '<queue>'…`, or `A worker claimed it and stopped making progress…`, naming the pollers that make it a measurement.
